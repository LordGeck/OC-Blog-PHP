...
class BlogPostManager extends Manager
{
    ...
    public function getBlogPost(int $blogPostId): array|false
    {
        $request = $this->database->prepare('SELECT id,
            ...
            main_content,
            firstname,
            SUBSTRING(lastname, 1, 1) AS lastname
            FROM blog_posts
            INNER JOIN users ON username = user_username
            WHERE id = ?');
    ...
    {
        $request = $this->database->prepare('INSERT INTO blog_posts
            (title, creation_date, update_date, header_content, main_content, user_username)
            VALUES(?, NOW(), NOW(), ?, ?, ?)');
    ...
    {
        $request = $this->database->prepare('UPDATE blog_posts
            SET title = ? , update_date = NOW() , header_content = ? , main_content = ? WHERE id = ?');
